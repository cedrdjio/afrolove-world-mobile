...
class OnbordingCubit extends Cubit<OnbordingState> {
  ...
  Future<UserModel> registerUserApi({
    ...
    try {
      ...
      print("MAMAMAMA${formData.fields}");
      ...
      if (response.statusCode == 200) {
        if (response.data["Result"] == "true") {
          ...
          OneSignal.User.addTagWithKey("user_id", response.data["UserLogin"]["id"]);
  ...
  Future forgotPassApi({required String mobile, required String password, required String ccode, required context}) async {
    try {
      Map data = {"mobile": mobile, "password": password, "ccode": "+$ccode"};
      Response response = await _api.sendRequest
  ...
  // Another Otp System
   ...
   SmaTypeApiModel? smaTypeApiModel;


  Future smstypeapi(context) async {
    try {
      Response response = await _api.sendRequest.get("${Config.baseUrlApi}${Config.smstypeapi}");

      var data1;
      if (response.data is String) {
        data1 = jsonDecode(response.data);
      } else {
        data1 = response.data;
      }
      ...
      if (response.statusCode == 200) {
        if (data1["Result"] == "true") {
          smaTypeApiModel = smaTypeApiModelFromJson(jsonEncode(data1));
          return data1;
        } else {
          Fluttertoast.showToast(msg: "${data1["ResponseMsg"]}");
        }
      } else {
        Fluttertoast.showToast(msg: "${data1["ResponseMsg"]}");
      }
  ...
  late MsgApiModel msgApiModel;

  Future msgotpapi({required String mobile, required context,}) async {
    ...
    try{
      var response = await _api.sendRequest.post("${Config.baseUrlApi}${Config.msgotpapi}",data: data);
      if(response.statusCode == 200){
        msgApiModel = MsgApiModel.fromJson(response.data);
  ...
  late TwilioApiModel twilioApiModel;

  Future twilyootp({required String mobile, required context,}) async {
    ...
    try{
      var response = await _api.sendRequest.post("${Config.baseUrlApi}${Config.twilyootpapi}",data: data);
      if(response.statusCode == 200){
        twilioApiModel = TwilioApiModel.fromJson(response.data);
